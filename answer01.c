...
int addElement(int * array, int length)
{
	...
	while (i !=length)
	{
		summ = summ + array[i]; //adding to the previos summ
		i++;
	}
	return summ;
...
int countNegative(int * array, int length)
{
	...
	if(length ==0)
	{
		return count; //if the array has no length count should be 0
	}
	while(i < length)
	{
		if(array[i] <0)
...
int isIncreasing(int * array, int length)
{
	int i = 0;
	int j = 0;
	int status = 1; //assume true
	for(i = 0; i<length && (status ==1);i++)
	{
		for(j=i+1;j<length;j++)   // u will need 2 loops to check.  the next one checks against all the previous numbers.
		{
			if(array[i]>array[j])
			{
				status = 0; //if the status has been changed there is no need to iterate through the rest. just break out of the loop
				break;
			}
		}
	}
